...
@Component({
  selector: 'app-hotel-list-meubles',
  templateUrl: './hotel-list-meubles.component.html',
  styleUrls: ['./hotel-list-meubles.component.css']
})
export class HotelListMeublesComponent implements OnInit {
    ...
    ngOnInit() {
      ...
      this.hotelListService.getHotels().subscribe({
        next: hotels => {
          this.hotels = hotels;
          this.filteredHotels = this.filterByCategorie();
        },
    ...
    public set hotelFilterVille(filter: string){
      ...
      this.filteredHotels = this.filterHotels();
    ...
    private filterHotels(): IHotel[]{
      ...
      this.filteredHotels = this.filterByCategorie(); // filtre initial

      //appliquons les fitres sur nos select
      if(this._hotelFilterVille) {
        this.filteredHotels = this.filteredHotels.filter((hotel:IHotel) => 
    ...
    public filterByCategorie(): IHotel[]{
      return this.hotels.filter((hotel: IHotel) => hotel.categorie === 'meublés');
    }
